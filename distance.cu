...
__global__ void compute_distance_matrix(const float* d_vectors, float* d_distance_matrix, int N, int M) {
...
void calculate_distance_matrix(const std::vector<std::vector<float>>& vectors) {
    ...
    cudaMemcpy(d_vectors, h_vectors, N * M * sizeof(float), cudaMemcpyHostToDevice);

    // Launch kernel with N threads
    int blockSize = 256; // Adjust block size as necessary
    int numBlocks = (N + blockSize - 1) / blockSize;
    compute_distance_matrix<<<numBlocks, blockSize>>>(d_vectors, d_distance_matrix, N, M);

    // Copy results back to host
    std::vector<std::vector<float>> distance_matrix(N, std::vector<float>(N));
    cudaMemcpy(distance_matrix[0].data(), d_distance_matrix, N * N * sizeof(float), cudaMemcpyDeviceToHost);
...
int main() {
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
            vectors[i][j] = static_cast<float>(rand() % 100); // Random values between 0 and 99
        }
    }
